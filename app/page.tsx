...
const Projects = () => {
  ...
  const prepareHtml = (item: any, dragType: string) => {
    ...
    html.id = item.id;
    html.innerHTML = `<div class='grid grid-cols-1 h-full'>
    <p>${item.name.length > 50 ? item.name.substring(0, 70) + "..." : item.name}</p>
    <div class='flex justify-center items-end'><img src='${item.image ? item.image : ""}' class='w-12 h-12 rounded-full pointer-events-none'/></div>
    </div>`;
    // html.innerText = ;
    html.className =
      "h-40 bg-[#78d5e3] m-2 hover:drop-shadow-2xl hover:cursor-move text-white p-2 overflow-hidden";
    html.draggable = true;
